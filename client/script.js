...
sayHelloButton.addEventListener('mouseout', changeBackBtn);
...
const repeatMyParam = (str) => {
    ...
    axios.get(`http://localhost:3000/repeat/${str}`)
    .then( res => {
...
const qRequest = () => {
    ...
    axios.get(`http://localhost:3000/query-test/?t=kajldsfjq=ljdaslkfjjad`)
    .then(res => {
        console.log(res.data)
    })
    .catch(err => {
...
document.getElementById('query-button').addEventListener('click', qRequest);
////////////////
//INTERMEDIATE//
////////////////
...
//error cannot set headers after they are sent to the client. also 'you sent an emppty query
...
document.getElementById('addFoodBtn').addEventListener('click', createFood)